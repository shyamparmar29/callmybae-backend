...
async def list_companions(db: AsyncSession = Depends(get_db), user: User = Depends(get_current_user)):
...
async def get_companion(companion_id: str, db: AsyncSession = Depends(get_db), user: User = Depends(get_current_user)):
    result = await db.execute(select(Companion).where(Companion.id == companion_id, Companion.user_id == user.id))
...
async def delete_companion(companion_id: str, db: AsyncSession = Depends(get_db), user: User = Depends(get_current_user)):
    result = await db.execute(select(Companion).where(Companion.id == companion_id, Companion.user_id == user.id))
    companion = result.scalar_one_or_none()
    if not companion:
        raise HTTPException(404, "Companion not found")
